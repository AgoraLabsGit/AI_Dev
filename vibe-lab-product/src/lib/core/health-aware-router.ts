...
export interface RouteConfig {
  ...
  fallbackResponse?: FlexibleObject;
}

export interface RoutingResult {
  service: string | null;
  isFallback: boolean;
  response?: FlexibleObject;
...
export class HealthAwareRouter {
   ...
   */
  async routeRequest(routeName: string, requestHandler: (service: unknown) => Promise<unknown>): Promise<unknown> {
      ...
      try {
        ...
        if (service) {
          ...
          if (typeof result === 'object' && result !== null) {
            (result as FlexibleObject)._routing = {
   ...
   */
  private getDefaultFallbackResponse(routeName: string, routingResult: RoutingResult): FlexibleObject {
   ...
   */
  getRouteStatus(routeName: string): FlexibleObject {
    const config = this.routes.get(routeName);
    if (!config) {
   ...
   */
  getAllRouteStatuses(): FlexibleObject {
    const statuses: FlexibleObject = {};
