...
import { IntegrationEventType } from '../dias/events/event-types';
...
export class IntegrationService extends BaseService {
  ...
  async syncState(projectId: string): Promise<void> {
    if (!this.integrationConfig.enableStateSync) {
  ...
  protected async healthCheck(): Promise<boolean> {
    ...
    const workerHealth = workerStatus.status === 'healthy';
  ...
  private async registerWorkerPools(): Promise<void> {
    ...
    if (this.integrationConfig.workerPoolSizes?.ai) {
      this.workerManager.registerWorkerPool('ai', {
        workerClass: ComponentGeneratorWorker as any,
        minWorkers: this.integrationConfig.workerPoolSizes.ai.min,
        maxWorkers: this.integrationConfig.workerPoolSizes.ai.max
      });
    }
      ...
      async (message) => {
        ...
        if (event.type === 'quality.check.failed') {
          await this.dias.emitIntegrationEvent(
            IntegrationEventType.DIAS_SUGGESTION,
            event.projectId,
            {
              sourceSystem: 'avca',
              targetSystem: 'dias',
...
class ComponentGeneratorWorker extends AIWorker {
  ...
  async processWorkerInput(input: WorkerInput, context: WorkerContext): Promise<WorkerOutput> {
    try {
      ...
      const result = await this.ai.generateCode(
        ...
        { 
          ...
          projectName: input.projectId // Use projectId as name if not available
        },
