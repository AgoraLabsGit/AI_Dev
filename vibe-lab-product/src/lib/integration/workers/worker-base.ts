...
export abstract class BaseWorker extends BaseService {
  ...
  /**
   * BaseService implementation - delegates to processWorkerInput
   */
  async process(input: any): Promise<any> {
    if (input && input.workerInput && input.workerContext) {
      return this.processWorkerInput(input.workerInput, input.workerContext);
    }
    throw new Error('Invalid input format for worker process method');
  }

  /**
   * Process worker input and return output
   */
  abstract processWorkerInput(input: WorkerInput, context: WorkerContext): Promise<WorkerOutput>;
  ...
  async execute(input: WorkerInput, context: WorkerContext): Promise<WorkerOutput> {
    ...
    try {
      ...
      const result = await this.withTimeout(
        this.processWorkerInput(input, context),
