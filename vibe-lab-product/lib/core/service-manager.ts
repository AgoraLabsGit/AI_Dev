...
export class ServiceManager {
  ...
  private createLazyProxy<T extends BaseService>(name: string, serviceFactory: () => T): T {
    return new Proxy({} as T, {
      get: (target, prop) => {
        ...
        if (actualService) {
          const value = (actualService as any)[prop];
        ...
        if (typeof prop === 'string') {
          return async (...args: any[]) => {
            ...
            if (readyService) {
              const method = (readyService as any)[prop];
            ...
            return this.getFallbackResponse(name, prop as string, args);
          };
        }
   ...
   */
  private getFallbackResponse(serviceName: string, method: string, args: any[]): any {
  ...
  private async checkServiceHealth(service: BaseService): Promise<boolean> {
    try {
      ...
      if ('isHealthy' in service && typeof service.isHealthy === 'function') {
        return await (service as any).isHealthy();
      }
