...
export class PerformanceOptimizationService extends BaseService {
  ...
  async cache<T>(key: string, value: T, options?: {
    ...
    try {
      ...
      this.log('debug', `Cached ${key} in ${level.name} (${entry.size} bytes, ${duration}ms)`);
  ...
  async get<T>(key: string): Promise<T | null> {
    ...
    try {
      ...
      for (const level of sortedLevels) {
        ...
        this.log('debug', `Cache hit for ${key} in ${level.name} (${duration}ms)`);
        return value as T;
      }
      ...
      this.log('debug', `Cache miss for ${key} (${duration}ms)`);
      return null;
  ...
  private async emitPerformanceEvent(type: string, entityId: string, data: any): Promise<void> {
    if (this.eventBus) {
      ...
      await this.eventBus.publish('performance', event);
    }
  }
}